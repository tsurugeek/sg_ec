...
RSpec.describe Cart, type: :model do
  ...
  describe "#update_product" do
    it "raises ActiveRecord::StaleObjectError when cart is old"
    context "when purchase products as same as argument's product exists," do
      before do
        cart.add_product(product1, 1)
        cart.update_product(product1, 5)
      end
      ...
      context "when the argument num is less than 1" do
        it "updates nothing with validation errors" do
          expect(cart.update_product(product1, 0)).to be false
          expect(cart.products_num).to eq 5
          expect(cart.errors['purchase_products.num'].size).to be > 0
        end
      end
    end
    context "when any purchase products don't exists," do
      before do
        cart.update_product(product1, 5)
      end
      it "doesn't occur validation errors" do
